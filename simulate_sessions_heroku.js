...
const BASE_TRAFFIC_PER_MINUTE = 40;
...
(async () => {
    for (let regression = 0; regression < 1; regression++) {
        ...
        let DB_NAME = `db_${DATE}_H_length_${SIMULATION_LENGTH_IN_MINUTES}_traffic_${BASE_TRAFFIC_PER_MINUTE}_regression_${REGRESSION_LEVEL}.db`;
        ...
        https.get(URL + `get_db`, resp => resp.pipe(fs.createWriteStream(new_db_name)));
